...
### Что такое процесс в Ergo Framework
...
Взаимодействие между процессами обеспечивается обменом сообщениями. Для этого можно использовать метод асинхронной отправки сообщений `gen.Process.Send(...)`. В этом случае в процессе-получателе будет вызван callback метод `HandleInfo(...)`. Однако, behavior `gen.Server` предоставляет два дополнительных метода
...
Однако, если необходимо обратиться к процессу из обычного golang-кода (не из другого процесса), для этого необходимо иметь интерфейс процесса `gen.Process`, который возвращается функцией создания нового процесса `Spawn`. Используя метод `gen.Process.Direct(...)` можно сделать синхронный запрос процессу. В этом случае в процессе-получателе будет вызван callback метод `HandleDirect(...)`.
